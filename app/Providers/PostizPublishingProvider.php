...
final class PostizPublishingProvider implements PublishingProvider
{
    public function publish(PublishingRequest $r): PublishingProviderResult
    {
        ...
        if ($key === '') {
            ...
            $x = Http::withToken($key)->acceptJson()->asJson()->timeout((int) config('services.postiz.timeout', 15))->withHeaders(['Idempotency-Key' => $r->idempotencyKey])->post(rtrim((string) config('services.postiz.url'), '/').'/posts', ['type' => 'schedule', 'date' => $r->scheduledAt, 'posts' => [['integration' => ['id' => $r->integrationId], 'value' => [['content' => $r->content, 'image' => []]], 'settings' => $r->settings]]]);
...
return new PublishingProviderResult($id, is_string(data_get($p, 'url')) ? data_get($p, 'url') : null, 'submitted', is_array($p) ? $p : []);
        }throw new PublishingProviderException('Postiz rejected the publication request.', $x->status() === 429 ? 'rate_limited' : ($x->serverError() ? 'provider_unavailable' : 'provider_rejected'), $x->status() === 429 || $x->serverError());
    }
}