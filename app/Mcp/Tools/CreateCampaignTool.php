...
#[Description('Create a campaign under an authorized enterprise and marketing strategy.')]
class CreateCampaignTool extends DomainMutationTool
{
    protected static function schemaFields(JsonSchema $schema): array
    {
        return ['enterprise_id' => $schema->integer()->min(1)->required(), 'marketing_strategy_id' => $schema->integer()->min(1)->required(), 'name' => $schema->string()->min(1)->max(255)->required(), 'description' => $schema->string()->max(10000), 'agent_assignment_id' => $schema->integer()->min(1), 'agent_execution_id' => $schema->integer()->min(1), 'approval_request_id' => $schema->integer()->min(1)];
    }

    protected static function rules(): array
    {
        return ['enterprise_id' => ['required', 'integer', 'min:1', 'exists:enterprises,id'], 'marketing_strategy_id' => ['required', 'integer', 'min:1', 'exists:marketing_strategies,id'], 'name' => ['required', 'string', 'min:1', 'max:255'], 'description' => ['nullable', 'string', 'max:10000'], 'agent_assignment_id' => ['nullable', 'integer', 'min:1', 'exists:agent_assignments,id'], 'agent_execution_id' => ['nullable', 'integer', 'min:1', 'exists:agent_executions,id'], 'approval_request_id' => ['nullable', 'integer', 'min:1', 'exists:approval_requests,id']];
    }
    ...
    {
        return ['enterprise_id' => $validated['enterprise_id'], 'marketing_strategy_id' => $validated['marketing_strategy_id']];
    }
    ...
    {
        return ['create', [Campaign::class, static::enterprise($validated)]];
    }

    protected static function mutate(User $actor, DomainResourceService $domain, array $validated, ?Model $target = null): Model
    {
        return $domain->createCampaign($actor, static::enterprise($validated), $validated);
    }
