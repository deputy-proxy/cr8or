...
it('scopes phase one resources to the authenticated users organizations',function(){
 $org=Organization::factory()->create(); $other=Organization::factory()->create(); $user=User::factory()->create();
...
it('allows owners to create enterprise records and members to view them',function(){
 $org=Organization::factory()->create(); $owner=User::factory()->create(); $member=User::factory()->create();
 ...
 $this->actingAs($owner); expect(ProductResource::canCreate())->toBeTrue();
 $this->actingAs($member); expect(ProductResource::canCreate())->toBeFalse();
});
